...
kokoro = OpenAI(base_url=os.environ.get("KOKORO_BASE_URL"), api_key="not-needed")
...
class ExtractedText(BaseModel):
    ...
    has_book_in_image: bool

    def __str__(self):
        book_in_image = "Yes" if self.has_book_in_image else "No"
        return (
            f"ExtractedText(extracted_text='{self.extracted_text}', "
            f"page_numbers={self.page_numbers}, has_book_in_image='{book_in_image}')"
        )
...
def extract_text_from_image(base64_image: str) -> ExtractedText:
    response = client.beta.chat.completions.parse(
        ...
        messages=[
            {
                "role": "system",
                "content": """You are a helpful assistant that extracts text from books. Extract 
                the text from the pages of the book specifically. If the text is cutoff, extract 
                it as normal. Don't miss any text. Don't make up any sentences. Don't include the 
                title of the book. Don't include the author of the book. If you don't see a book 
                in the image, then has_book_in_image should be False. BUT, if you do see a legible
                book, then has_book_in_image should be True.""",
            },
            {
                "role": "user",
                "content": [
...
def analyze_mood(chunk: str) -> SentimentAnalysis:
    ...
    Provide the temperature, RGB, and other parameters as needed. Try to think about how to make the
    text feel immersive for a visually impaired person. The text is from a page of a book.

    Temperature: -1.0 to 1.0 (-1 is hottest, 1 is coldest). Do it in increments of .25.
        ...
        messages=[
            {
                "role": "system",
                "content": """
                You are a sentiment analysis assistant that helps people that are visually impaired 
                feel immersive haptics based on text. The text is from a page of a book. Always 
                respond in valid format.
                """,
            },
